...
async function main() {
  ...
  // Load existing candidates so we can de-dupe by videoId
  let existing = [];
  if (fs.existsSync(CANDIDATES_PATH)) {
    try {
      existing = JSON.parse(fs.readFileSync(CANDIDATES_PATH, "utf8"));
  ...
  const existingIds = new Set(existing.map((c) => c.videoId));
  const newCandidates = [];
    ...
    for (const term of searchTerms) {
      ...
      for (const r of results) {
        if (!existingIds.has(r.videoId)) {
          ...
          console.log(`    (already in candidates: ${r.videoId})`);
        }
      }
  ...
  // Merge new with existing, newest first
